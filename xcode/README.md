...
The icon is not hand-drawn either. `tools/make_icon.py` runs the same particle
physics with the same constants and the same `mulberry32`, so the icon is
literally a frame of this fire, framed for a square and rendered at 2x then
downsampled. The logs are signed-distance capsules. Re-run it any time:

```bash
python3 tools/make_icon.py     # needs numpy and pillow
```
